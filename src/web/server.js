...
export function createWebServer(client) {
  ...
  app.get('/callback', async (req, res) => {
    ...
    // Discord returned an error (e.g. user cancelled, invalid scope, etc.)
    if (error) {
      console.error(`Discord OAuth2 error: ${error} — ${error_description}`);
      return res.redirect(`/error?reason=discord_error&desc=${encodeURIComponent(error_description || error)}`);
    }
    ...
    try {
      ...
      // 7. Send welcome DM to user (dm_channels.messages.write — bot permission, works automatically)
      try {
        const dmChannel = await client.users.createDM(user.id);
        await dmChannel.send({
          embeds: [{
            title: '🎉 تم تفعيل حسابك في South Group!',
            description:
              '> أهلاً وسهلاً بك في مجتمعنا!\n\n' +
              '✅ تم التحقق من حسابك بنجاح\n' +
              '🏅 تم منحك دور المُفعَّل\n' +
              '🚀 يمكنك الآن الوصول لجميع قنوات السيرفر\n\n' +
              '> نتمنى لك وقتاً ممتعاً! 💙',
            color: 0x57F287,
            footer: { text: 'South Group • نظام التحقق' },
            timestamp: new Date().toISOString(),
          }],
        });
        console.log(`📩 Welcome DM sent to ${user.id}`);
      } catch (_) {
        // User may have DMs disabled — not a critical error
        console.log(`⚠️ Could not send DM to ${user.id} (DMs might be disabled)`);
      }

      // 8. Save user to session and redirect to success
      ...
      console.error('  ENV check — CLIENT_ID:', !!process.env.CLIENT_ID,
        '| CLIENT_SECRET:', !!process.env.CLIENT_SECRET,
        '| REDIRECT_URI:', process.env.REDIRECT_URI,
